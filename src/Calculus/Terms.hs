module Calculus.Terms(
         ...
         applySubst, freeVariables, constructors, splitArgs
       ) where
...
constructors :: Term -> S.Set QName
constructors (Var name)          = S.empty
constructors (Cons name)         = S.singleton name
constructors (ConstInt _)        = S.empty
constructors (ConstChar _)       = S.empty
constructors (Fresh name t)      = constructors t
constructors (Lam name p)        = constructorsP p
constructors (LamL loc name p)   = constructorsP p
constructors (Fix name t)        = constructors t
constructors (App t1 t2)         = constructors t1 `S.union` constructors t2
constructors (Seq t1 t2)         = constructors t1 `S.union` constructors t2
constructors (Unif t1 t2)        = constructors t1 `S.union` constructors t2
constructors (Function _ terms)  = S.unions $ map constructors terms
constructors (Command _ terms)   = S.unions $ map constructors terms

constructorsP :: Program -> S.Set QName
constructorsP Fail = S.empty
constructorsP (Alt t p) = constructors t `S.union` constructorsP p
